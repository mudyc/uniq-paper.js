...
#define ds_curve(t) ( 6. * t * (1. - t) ) // derivative of s_curve
...
#define at2(rx,ry) ( rx * q[0] + ry * q[1] )
    ...
    /** A version of noise2 with analytically computed derivatives 
     * Added by jvk */
    float noise2(float vec[2], float der[])
    {
	int bx0, bx1, by0, by1, b00, b10, b01, b11;
	float rx0, rx1, ry0, ry1, *q, sx, sy, a, b, t, u0, v0, u1, v1;
	register int i, j;
	...
	q = g2[ b00 ] ; u0 = at2(rx0,ry0);
	q = g2[ b10 ] ; v0 = at2(rx1,ry0);
	a = lerp(sx, u0, v0);

	q = g2[ b01 ] ; u1 = at2(rx0,ry1);
	q = g2[ b11 ] ; v1 = at2(rx1,ry1);
	b = lerp(sx, u1, v1);

	// Analytic derivative
	if (der) {
	  float dsy = b - a;
	  float dsx = (v0 - u0) * (1 - sy) + (v1 - u1) * sy;
	  float du0 = (1 - sx) * (1 - sy);
	  float dv0 = sx * (1 - sy);
	  float du1 = (1 - sx) * sy;
	  float dv1 = sx * sy;
	  ...
	  der[0] =
	    g2[ b00 ][ 0 ] * du0 + g2[ b10 ][ 0 ] * dv0 +
	    g2[ b01 ][ 0 ] * du1 + g2[ b11 ][ 0 ] * dv1 +
	    ds_curve(rx0) * dsx;
	  der[1] =
	    g2[ b00 ][ 1 ] * du0 + g2[ b10 ][ 1 ] * dv0 +
	    g2[ b01 ][ 1 ] * du1 + g2[ b11 ][ 1 ] * dv1 +
	    ds_curve(ry0) * dsy;
	}
